...
def deps():
    ...
    if "ubuntu1604_java_debs" not in excludes:
        http_file(
            ...
            urls = [
                "https://storage.googleapis.com/container_release_tools_e2e_tests/bazel_docker_gcloud_staging/java/debs/" + JAVA_DEBS_TARBALL.revision + "_java_debs.tar",
            ],
        )
    ...
    if "ubuntu1604_java_cacerts" not in excludes:
        http_file(
            ...
            urls = [
                "https://storage.googleapis.com/container_release_tools_e2e_tests/bazel_docker_gcloud_staging/java/cacerts/" + CACERTS.revision + "_cacerts.tar",
            ],
        )
